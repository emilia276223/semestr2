...
(define fix-funct
  (lamE 'f
        (letE 'w (lamE 'g (appE (varE 'f)
                                (lamE 'z (appE (appE (varE 'g)
                                                     (varE 'g))
                                               (varE 'z)))))
              (appE (varE 'w) (varE 'w)))))
  ...
  (cond
    ...
    [(s-exp-match? `{letrec SYMBOL ANY ANY} s)
     (letE (s-exp->symbol (second (s-exp->list s)))
           (appE fix-funct (lamE (s-exp->symbol (second (s-exp->list s)))
                                 (parse (third (s-exp->list s)))))
     (parse (fourth (s-exp->list s))))]
...
;operator punktu stałego umożliwa obliczenie wyrażeń rekurencyjnych w języku bez takich wyrażeń,
;używamy cały czas takiego samego operatora, umożliwia na to tworzenie rekurencji
...
#;(run `{letrec x {+ x 1} x}) ;błąd, bo zakładamy, ze w letrecu jest lambda
