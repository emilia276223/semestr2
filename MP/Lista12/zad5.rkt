...
(define-type Value
  ...
  (recV [f : Symbol] [x : Symbol] [e : Exp] [env : Env]) ;dodaję rekurencję, jako wartość
  )
...
(define (eval [e : Exp] [env : Env]) : Value
  (type-case Exp e
    ...
    ;ewaluacja rekurencji ogranicza się do stworzenie wartości rekurencyjnej
    [(recE f x e)
     (recV f x e env)]))
...
(define (apply [v1 : Value] [v2 : Value]) : Value
  (type-case Value v1
    ...
    [(primopV f)
     (f v2)]
    ;aplikacja dla rekurencji
    ;ewaluujemy wyrażenie e ze środowiskiem dla funkcji i jej
    ;wywołania rekurencyjnego, dla zmiennej której środowisko
    ;zostało powiększone o środowisko v2
    [(recV f x e env)
     (eval e (extend-env (extend-env env x v2)
                         f (recV f x e env)))]
    ;dlaczego to dziala? bo jak mamy gdzies uzyta te rekurencyjna
    ;funkcje to mamy tam apply wiec zachowa on sie rekurencyjnie,
    ;dzieki temu ze if ewaluuje tylko to co trzeba to wszystko
    ;bedzie dobrze
...
;1. moze tam zostac wpisana zmienna a nie funkcja => nie
;mozna tego traktowac jako funkcji i nie bedzie apply,
;wiec rekurencja zwyczajnie nie zadziala
