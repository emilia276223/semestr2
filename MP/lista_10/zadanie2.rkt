...
(define (decomp [code : Code] [es : (Listof Exp)]) : Exp
  (if (empty? code)
      (if (empty? es)
          (error 'decompile "empty imput")
          (if (empty? (rest es))
              (first es)
              (error 'decompile "wrong imput")))
      (type-case Instr (first code)
        [(pushI n) (decomp (rest code) (cons (numE n) es))]
        [(opI op) (cond [(empty? es) (error 'decomp "wrong imput")]
                        [(empty? (rest es)) (error 'decomp "wrong imput")]
                        [else (decomp (rest code) (cons (opE op (second es) (first es)) (rest (rest es))))])])))
...
(module+ test
  ...
  (test (decompile (compile (parse `{+ {* 2 3} {+ 5 8}})))
        (parse `{+ {* 2 3} {+ 5 8}}))
  (test (decompile (compile (parse `{+ {* 2 {+ 3 4}} {+ 5 8}})))
        (parse `{+ {* 2 {+ 3 4}} {+ 5 8}}))
  (test (decompile (compile (parse `{+ {* {+ {- 2 3} 3} 4} {+ 5 8}})))
        (parse `{+ {* {+ {- 2 3} 3} 4} {+ 5 8}}))
  (test (decompile (compile (parse `{+ {+ {- {+ {* 5 8} 4} 5} 3} 2})))
        (parse `{+ {+ {- {+ {* 5 8} 4} 5} 3} 2})))
