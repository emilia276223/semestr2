...
(define (bind2 [a1 : Answer] [a2 : Answer] [f : (Value Value -> Answer)]) : Answer
  (type-case Answer a1
    [(valueA v1)
     (type-case Answer a2
       [(valueA v2) (f v1 v2)]
...
(define (op-num-num->proc [f : (Number Number -> Number)]) : (Answer Answer -> Answer)
  (λ (a1 a2)
    (bind2 a1 a2
           (λ (v1 v2)
       (type-case Value v1
...
(define (div->proc [f : (Number Number -> Number)]) : (Answer Answer -> Answer)
  (λ (a1 a2)
    (bind2 a1 a2
           (λ (v1 v2)
             (type-case Value v1
               [(numV n1)
                (type-case Value v2
                  [(numV n2)
                   (if (not (= n2 0))
                       (return (numV (f n1 n2)))
                       (err 'eval "attempt to divide by 0"))]
                  [else
                   (err 'eval "type error")])]
               [else
                (err 'eval "type error")])))))

(define (op-num-bool->proc [f : (Number Number -> Boolean)]) : (Answer Answer -> Answer)
  (λ (a1 a2)
    (bind2 a1 a2
           (λ (v1 v2)
             (type-case Value v1
...
(define (eval [e : Exp]) : Answer
  (type-case Exp e
    ...
    [(opE o l r)
     (do ([v-l (eval l)]
          [v-r (eval r)])
       ((op->proc o) (valueA v-l) (valueA v-r)))]
    [(ifE b l r)
     (do ([v (eval b)])
       (type-case Value v
         [(boolV u)
